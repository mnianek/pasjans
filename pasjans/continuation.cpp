...
struct waist			//elementy generowania karty
{
	...
	char number = '0';		// od 0 do 12 (talia składa się z 13 kart) A = 14, K=13, 12=D, 11=J, 2-10
	bool view;			// odsłonięta (true) lub zakryta (false)
};
...
void cards::create_cards()
{
	...
	srand(time(NULL));
	int lp = 52;			// zmienna mówiąca ile razy ma się powtórzyć petla (52 na talie + błędy)
...
void cards::table() {
	//******
	//1)Tworzenie stołu w formie tablica
	...
	//sort(tab, tab + 28);	//sprawdzenie czy działa
...
//	            
	...
	for (int i = 0; i < 7; i++) {
		int ng = 6;	//zmienna do zmiany komorek w wierszu (tylko gorna część)									-----
		int ns = 5; //zmienna do zmiany komorek w wierszu (tylko środkowa część) || 5 zamiast 6 dla wyglądu	   |     |
		int nd = 6;	//zmienna do zmiany komorek w wierszu (tylko dolna część)								   |     |
		for (int j = 0; j < 7; j++) {
			if (karta[i][j].numKr > 0&& karta[i][j].view==true) 
			{
				
				//********************************************************************
				//karta ma 6 wysokosci, 7 szerokości
				...
				for (int WysokoscPojKarty = (i * 6) + 2; WysokoscPojKarty < wysokosc; WysokoscPojKarty++) {
					...
					for (int i2 = ns + 1; i2 < ns + 6; i2++)
					{
						...
						{
							if (i2 == (ns + 3) && wysokosc - WysokoscPojKarty == 2) wyglad[WysokoscPojKarty][i2] = '0'; //pokazuje wartość karty
							if (i2 == (ns + 2) && wysokosc - WysokoscPojKarty == 2) wyglad[WysokoscPojKarty][i2] = '1'; //pokazuje wartość karty
						}
						else
						{
							if (i2 == (ns + 3) && wysokosc - WysokoscPojKarty == 2) wyglad[WysokoscPojKarty][i2] = card[ktora_karta].number; //pokazuje wartość karty
						}
